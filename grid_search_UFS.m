...
%% SIM Reconstruction
snr_array = zeros(length(lamb_TV), length(lamb_hess));
for ii=1:length(lamb_TV)
    for jj = 1 : length(lamb_hess)
        FF=[Fn, {lamb_TV(ii)*F_TV}, {lamb_hess(jj)*F_hess}, {pos}];
        HH=[Hn, {Op_TV}, {Op_hess}, {OpD}];
        rho=[ones(size(Fn))*rhoDTNN,rho_TV(ii),rho_hess(jj),rhoDTNN];
        % Solver for ADMM
        A = rho_TV(ii) * Op_TV.makeHtH + rho_hess(jj) * Op_hess.makeHtH + rhoDTNN * valId * LinOpIdentity(sz);
        ...
        snr_array(ii, jj) = OptSNR(im, xopt);
        if sav
            if ~isempty(pattname)
...
[~, I] = max(snr_array);
FF=[Fn, {lamb_TV(I(1))*F_TV}, {lamb_hess(I(2))*F_hess}, {pos}];
HH=[Hn, {Op_TV}, {Op_hess}, {OpD}];
rho=[ones(size(Fn))*rhoDTNN,rho_TV(ii),rho_hess(jj),rhoDTNN];
% Solver for ADMM
A = rho_TV(ii) * Op_TV.makeHtH + rho_hess(jj) * Op_hess.makeHtH + rhoDTNN * valId * LinOpIdentity(sz);
