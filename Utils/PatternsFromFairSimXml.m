...
%% Extract parameters
band=str2double(strrep(st.fairsim.sim_dash_param.nr_dash_bands.Text, ',', '.'));
res_xy=str2double(strrep(st.fairsim.sim_dash_param.microns_dash_per_dash_pxl.Text, ',', '.')) * 1000;   % Lateral resolution (converted to nm)
...
k0=0;
for ii=1:nbOrr
    phOff(ii)=str2double(strrep(getfield(getfield(getfield(st.fairsim.sim_dash_param,['dir_dash_',num2str(ii-1)]),'phase_dash_offset'),'Text'), ',', '.'));
    shift=str2num(strrep(getfield(getfield(getfield(st.fairsim.sim_dash_param,['dir_dash_',num2str(ii-1)]),'shift'),'Text'), ',', '.'));
    k0(ii)=norm(shift);
